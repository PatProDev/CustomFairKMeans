...
class HarmonicKMeans(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X, sensitive_feature):
        ...
        for iteration in range(self.max_iters):
            ...
            new_centroids = np.array([self._harmonic_mean(X[labels == i]) if np.any(labels == i) else self.cluster_centers_[i] for i in range(self.n_clusters)])
    ...
    def _harmonic_mean(self, points):
        """
        Compute harmonic mean of points along each feature dimension.
        """
        if len(points) == 0:
        ...
        eps = 1e-9
        denom = np.sum(1.0 / (points + eps), axis=0)
        harmonic = n / denom
    ...
    def predict(self, X):
        ...
        """
        distances = np.array([np.linalg.norm(X - centroid, axis=1) for centroid in self.cluster_centers_])
